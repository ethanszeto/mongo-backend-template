...
describe("Default Router Endpoint Tests", () => {
  test("Test connecting to the API", async () => {
    ...
    expect(JSON.stringify(response.body)).toBe(JSON.stringify({ message: "Successfully connected to the API!" }));
  });
  ...
  test("Test connecting to the MongoDB instance", async () => {
    ...
    showLog && console.log(response.body);
    expect(response.statusCode).toBe(200);
    expect(JSON.stringify(response.body)).toBe(JSON.stringify({ message: "Successfully connected to the DB through the API!" }));
  });
});
